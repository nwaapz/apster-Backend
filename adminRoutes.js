...
      // --- Pool balance ---
      try {
        const poolBN = await contract.poolBalance?.();
        contractInfo.balance = poolBN
          ? (typeof poolBN === 'bigint') // Check for native BigInt
            ? ethers.formatEther(poolBN) // Direct access to formatEther
            : String(poolBN)
          : "0";
      } catch (err) {
...
          depositBN = depositBN ?? 0n; // Use native BigInt literal
          playerDeposits[addr] = (typeof depositBN === 'bigint')
            ? ethers.formatEther(depositBN) // Direct access to formatEther
            : String(depositBN);
        } catch (err) {
